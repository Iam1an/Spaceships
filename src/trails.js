...
// Cached geometries — one per shape, shared across all particles of that type.
const geoCache = {};
...
function buildGeometry(shape) {
  switch (shape) {
    ...
    case 'david': {
      ...
      const r = 0.5;
      const h = r * Math.sqrt(3) / 2; // r × sin(60°)
...
// Flat shapes need to be billboarded toward the camera each frame.
...
export function createTrails() {
  ...
  // camera is optional; when provided, flat particles billboard toward it.
