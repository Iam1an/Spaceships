...
export function initCustomizationScene(canvasEl) {
  ...
  // Glowing ring on top of pedestal — pulses with hull color
  ...
  // Faint floor disc under pedestal
  ...
  // Subtle spotlight cone on the floor
  ...
  // Hull and accent mesh buckets — populated after model loads
  const hullMeshes   = [];
  const accentMeshes = [];
  ...
  // Show pedestal immediately, add ship once model resolves
  running = true;
  tick();
  ...
  loadShipModel()
    .catch(() => {})
    .finally(() => {
      // Load without a tint so original material colors are preserved for classification
      const ship = createShip();
      ship.scale.setScalar(2.5);
      shipGroup.add(ship);

      // Center the ship's bounding box at the group origin so it spins around its true center
      ...
      // Classify meshes: name-based for the GLB (Cockpit / Engines → accent),
      // luminance fallback for the procedural ship (accentMat is clearly darker).
      ship.traverse(o => {
