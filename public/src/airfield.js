...
export function createAirfield(team = 0) {
  ...
  const RW = 560, RD = 380, RH = 3;   // grey base for the air thing
  const tarmac = new THREE.Mesh(new THREE.BoxGeometry(RW, RH, RD), tarmacMat);
  tarmac.position.y = -RH / 2;   // top face at y=0
  group.add(tarmac);
  const cl = new THREE.Mesh(new THREE.BoxGeometry(4, 0.05, RD * 0.82), lineMat);  // runway
  group.add(cl);
  for (const endZ of [-RD * 0.38, RD * 0.38]) {   // Threshold bars (8 short dashes at each end)
    for (let k = -3; k <= 3; k++) {
  ...
  group.add(radar);
  for (const [hx, hz] of [[-RW * 0.3, RD * 0.32], [RW * 0.3, RD * 0.32]]) { // hanger
  ...
  const tankMat = new THREE.MeshStandardMaterial({ color: 0x888866, roughness: 0.7 }); // tanks
  ...
  const pLight = new THREE.PointLight(accentColor, 2.0, 200); // outside lights
